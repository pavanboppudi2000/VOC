...
# object_categories = np.array(['阴茎', '外阴', '下体', '乳房',
#                      '臀部', '性交', '给女性口交', '给男性口交', '给女性手交',
#                      '给男性手交', '无性器官性交动作', '无性器官口交动作', 
#                      '足交', '乳交', '道具',
#                      '颜射', '比基尼', '短裙', '拥抱', '亲吻', '牵手'])
object_categories = np.array(['aeroplane', 'bicycle', 'bird', 'boat',
...
class Engine(object):
    ...
    def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        # record loss
        self.state['loss_batch'] = self.state['loss'].item() # data[0] -> item()
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            # measure accuracy
            self.on_end_batch(True, model, criterion, data_loader, optimizer,True)
    ...
    def validate(self, data_loader, model, criterion):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            # if save_results:
            #     res=self.state['output'].cpu().numpy()>0 
            #     for id_m,r in enumerate(res):
            #         pre=object_categories[r==True]
            #         path=input[1][id_m].cpu().numpy()
            #         folder='../porn_data/porn'+ str(path//10000)
            #         image_name="/%06d"% (path) + '.jpg'
            #         image_path=folder+image_name
            #         if pre.size==0:
            #             print('no class found: '+image_path)
            #             continue
            #         with open('./results/'+image_name+'.txt','w',encoding='utf-8') as f:
            #             for p in pre:
            #                 f.write(p+'\n')
            #         shutil.copy(image_path,'./results/')

            if save_results:
                res=self.state['output'].cpu().numpy()>0 
                for id_m,r in enumerate(res):
                    pre=object_categories[r==True]
                    folder='/home/zhangjunjie/MobileNetV2-master/image_140/images_highrisk/'
                    image_name=input[1][id_m]
                    image_path=folder+image_name
                    if pre.size==0:
                        #print('no class found: '+image_name)
                        # shutil.copy(image_path,'./results_5w/')
                        continue
                    #with open('./results_100w/'+image_name+'.txt','w',encoding='utf-8') as f:
                        #for p in pre:
                            #f.write(p+'\n')
                    #shutil.copy(image_path,'./results_100w/')
...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        ap_per_label = self.state['ap_meter'].value().numpy()
    ...
    def on_start_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if self.state['save_model_path'] != 'checkpoint/porn/':
